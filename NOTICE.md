...
Pinned runtime packages in [requirements.txt](requirements.txt) are rhino3dm 8.35.0, mcp 1.30.0, Pillow 11.3.0 and numpy 2.3.3. Development tools in [requirements-dev.txt](requirements-dev.txt) are mypy 1.18.2 and ruff 0.16.2. Installed distribution metadata previously showed permissive and notice-bearing licenses, including MIT, BSD-family, Apache-2.0 and MPL-2.0 among transitive packages. This is not a complete legal review of every wheel, binary or optional deployment artifact. 고정 의존성의 설치 메타데이터를 확인했지만 실제 배포물의 모든 전이 패키지·네이티브 바이너리·고지 의무를 완전히 확인한 것은 아닙니다.

The repository does not bundle Rhino or Blender executables. It may invoke a separately installed Blender for optional rendering/import. User-supplied photographs, plans, marks, fonts, models, prompts and AI-generated work are not cleared by this project's license. Rhino나 Blender 실행 파일은 동봉하지 않으며, 사용자 제공 사진·도면·상표·폰트·모델·프롬프트·AI 생성물의 권리는 별도 확인이 필요합니다.
