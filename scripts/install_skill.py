...
def install(host: str, project: Path, dry_run: bool = False) -> dict:
    ...
    server_status = _codex_status() if host == "codex" else ("missing" if _claude_config(project)[2] else "ready")
    _ensure_runtime(dry_run)
    if dry_run:
        return {"host": host, "skill": str(destination), "skill_status": "exists" if destination.exists() else "would_copy", "mcp_status": server_status, "dry_run": True}
...
def main() -> None:
    ...
    print(json.dumps(install(args.host, args.project, args.dry_run), ensure_ascii=False, indent=2))
